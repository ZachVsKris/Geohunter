...
export default function AccountLeaderboard({ challengeDate, assignments, completed, difficulty = "easy" }: { challengeDate: string; assignments: Record<string,string>; completed: boolean; difficulty?: "easy" | "expert" }) {
  ...
  useEffect(() => {
    if (!open) return;
    fetch(`/api/leaderboard?difficulty=${difficulty}`).then((response)=>response.json()).then((data)=>setLeaders(data.leaders ?? [])).catch(()=>setLeaders([]));
  }, [open]);
  ...
  async function saveScore() {
    const response = await fetch("/api/scores", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ challengeDate, difficulty, assignments }) });
