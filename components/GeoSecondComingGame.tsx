...
function tryAssembleRound(available: RoundCategory[], countryList: CountryInfo[], rng: Rng, attempts: number): Round | null {
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    const sourceRoundCounts = new Map<string, number>();
    for (const category of shuffle(available, rng)) {
      if ((counts.get(category.category.family) ?? 0) >= 2) continue;
      if (category.category.source === "faostat" && (sourceRoundCounts.get("faostat") ?? 0) >= 2) continue;
      categories.push(category);
      counts.set(category.category.family, (counts.get(category.category.family) ?? 0) + 1);
      sourceRoundCounts.set(category.category.source, (sourceRoundCounts.get(category.category.source) ?? 0) + 1);
...
async function buildRound(countryList: CountryInfo[], seed: string): Promise<Round> {
  ...
  // Keep each batch diverse, but avoid letting slower FAOSTAT requests dominate
  // first paint. The order remains deterministic for a given seed.
  const fast = shuffledCategories.filter((category) => category.source !== "faostat");
  const faostat = shuffledCategories.filter((category) => category.source === "faostat");
  const scheduledCategories: Category[] = [];
  while (fast.length || faostat.length) {
    scheduledCategories.push(...fast.splice(0, 7));
    scheduledCategories.push(...faostat.splice(0, 1));
  }
  ...
  for (let offset = 0; offset < Math.min(scheduledCategories.length, MAX_CANDIDATES); offset += BATCH_SIZE) {
    ...
    if (available.length >= 8) {
      const earlyRound = tryAssembleRound(available, countryList, rng, 28);
  ...
  const finalRound = tryAssembleRound(available, countryList, rng, 120);
...
export default function GeoSecondComingGame() {
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""}`}>
    ...
    {status && <div className="loading"><div className="spinner"/><strong>{status}</strong><span>Only a small candidate set is loaded. Previously loaded datasets and completed rounds are reused across visits for faster play.</span></div>}
    ...
    <section className="dataNote"><strong>Current library: {CATEGORIES.length} rigorously sourced category definitions</strong><p><a href="/data">Data & methodology</a> · <a href="/privacy">Privacy</a> · <a href="/terms">Terms</a></p><p>World Bank, FAOSTAT, WHO, UNESCO UIS, and UN Tourism categories share the same ranking and contradiction checks. A hard recency gate excludes every country observation before 2022; any category that cannot still meet its coverage floor is automatically removed from playable rounds. Natural Earth is registered but contributes no playable categories because timeless geometry cannot satisfy that rule.</p></section>

    {showRules&&<div className="modal" onClick={(e)=>e.currentTarget===e.target&&setShowRules(false)}><div><h2>How Geo: Second Coming works</h2><ol><li>Choose the shared daily challenge or generate a random challenge with its own shareable seed.</li><li>Eight categories are selected deterministically from that seed, with no more than two from the same subject family.</li><li>Each category winner is drawn from that category’s global top 100.</li><li>Within the ten-country pool, eight different countries are each #1 for exactly one of the eight categories. The other two are decoys.</li><li>Assign eight countries and discard two. Each country can be used once.</li><li>Anyone opening the same challenge link receives the same categories, countries, and category order.</li></ol><p>Enabled category values are loaded from their named official source: World Bank WDI, FAOSTAT, WHO, UNESCO UIS, or UN Tourism. The daily challenge changes at midnight UTC. Challenge links preserve the seed, dataset release, category set, and rules version.</p><button onClick={()=>setShowRules(false)}>Start drafting</button></div></div>}
  </div>;
}
