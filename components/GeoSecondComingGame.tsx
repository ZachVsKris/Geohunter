...
${url}`);
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""} ${scores ? "resultsView" : ""}`}>
    ...
    {showRules&&<div className="modal" onClick={(e)=>e.currentTarget===e.target&&setShowRules(false)}><div><h2>How GeoStats works</h2><ol><li><strong>Assign eight countries.</strong> Each round has 10 countries and 8 categories. Place one country in each category. Two countries will be left unused.</li><li><strong>Read the category direction.</strong> For “Highest,” the largest value ranks first. For “Lowest,” the smallest value ranks first.</li><li><strong>Use each country once.</strong> A country can only be assigned to one category.</li><li><strong>Earn points based on rank.</strong> Your chosen country is ranked against the other nine countries in the round: 1st earns 100 points, 2nd earns 90, 3rd earns 80, and so on down to 10 points for 10th. The highest possible score is <strong>800 points</strong>.</li><li><strong>Every round can be solved perfectly.</strong> Each category has a different country that ranks first among the 10 countries in that round. So all eight categories can be matched with a first-place country, and a score of <strong>800 is always possible</strong>.</li></ol><button onClick={()=>setShowRules(false)}>Start drafting</button></div></div>}
  </div>;
}
