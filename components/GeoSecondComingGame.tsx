...
export default function GeoSecondComingGame() {
  ...
  async function copyChallenge() {
    try {
      const rankCounts = scores?.reduce<Record<number, number>>((counts, row) => {
        counts[row.rank] = (counts[row.rank] ?? 0) + 1;
        return counts;
      }, {}) ?? {};
      const rankSummary = Array.from({ length: 10 }, (_, index) => index + 1)
        .filter((rank) => rankCounts[rank])
        .map((rank) => `${rank}${rank === 1 ? "st" : rank === 2 ? "nd" : rank === 3 ? "rd" : "th"}×${rankCounts[rank]}`)
        .join(" · ");
      const topFive = scores?.filter((row) => row.rank <= 5).length ?? 0;
      const text = scores
        ? `🌍 GEO: SECOND COMING — ${total}/800

Placements
${rankSummary}
Top 5: ${topFive}/8

Play this exact challenge:
${window.location.href}`
      ...
      setError("The score could not be copied automatically. Copy the URL from your browser instead.");
    }
  }
