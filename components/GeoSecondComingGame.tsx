  ...
  async function loadDailyRound(nextDifficulty: DailyDifficulty, existingCountries = countries) {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (saved?.easy?.encoded_board && saved?.expert?.encoded_board) {
            const restoredEasy = decodeRound(saved.easy.encoded_board, existingCountries);
            const restoredExpert = decodeRound(saved.expert.encoded_board, existingCountries);
            if (roundMatchesDifficulty(restoredEasy, "easy") && roundMatchesDifficulty(restoredExpert, "expert")) {
              finalPair = { easy: restoredEasy, expert: restoredExpert };
            }
          }
        }
      } catch {
      ...
      if (!roundMatchesDifficulty(finalRound, nextDifficulty)) {
        throw new Error(nextDifficulty === "easy"
          ? "The Normal Daily could not be repaired to 8 countries and 6 categories. Reload and try again."
          : "The Expert Daily could not be verified as 10 countries and 8 categories. Reload and try again.");
      }
      setRound(finalRound);
