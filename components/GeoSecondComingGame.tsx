...
${challengeUrl()}`;
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""} ${scores ? "resultsView" : ""}`}>
    ...
    <section className="challengeBar">
      ...
      <div className="challengeActions"><span className="mobileProgress">{Object.keys(assignments).length}/8 assigned</span>{scores && <button className="resultsRulesLink" onClick={() => setShowRules(true)}>Rules</button>}<button onClick={copyChallenge} disabled={!round}>{copied ? "Link copied ✓" : "Copy link"}</button></div>
    </section>
