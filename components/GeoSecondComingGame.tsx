...
import { fetchCategories } from "../lib/dataSources";
...
type ExactRoundSpec = { categoryIds: string[]; countryIds: string[] };
...
async function loadCandidateDatasets(seed: string): Promise<RoundCategory[]> {
  ...
  for (let offset = 0; offset < shuffled.length && loaded.length < 16; offset += batchSize) {
    ...
    const datasets = await fetchCategories(batch);

    for (const category of batch) {
      const raw = datasets.get(category.id);
      if (!raw) continue;
      const dataset = canonicalizeDataset(raw);
...
function parseExactRoundSpec(params: URLSearchParams): ExactRoundSpec | null {
  const categoryIds = (params.get("roundCats") ?? "").split(",").filter(Boolean);
  const countryIds = (params.get("bank") ?? "").split(",").filter(Boolean);
  if (categoryIds.length !== 8 || countryIds.length !== 10) return null;
  if (new Set(categoryIds).size !== 8 || new Set(countryIds).size !== 10) return null;
  const knownCategoryIds = new Set(CATEGORIES.map((category) => category.id));
  if (categoryIds.some((id) => !knownCategoryIds.has(id))) return null;
  return { categoryIds, countryIds };
}

async function buildExactRound(countryList: CountryInfo[], spec: ExactRoundSpec): Promise<Round> {
  const categoryById = new Map(CATEGORIES.map((category) => [category.id, category]));
  const countryById = new Map(countryList.map((country) => [country.id, country]));
  const definitions = spec.categoryIds.map((id) => categoryById.get(id));
  const bank = spec.countryIds.map((id) => countryById.get(id));

  if (definitions.some((category) => !category) || bank.some((country) => !country)) {
    throw new Error("This challenge link contains an unknown category or country.");
  }

  const exactDefinitions = definitions.filter((category): category is Category => Boolean(category));
  const datasets = await fetchCategories(exactDefinitions);
  const missing = exactDefinitions.filter((category) => !datasets.has(category.id));
  if (missing.length) throw new Error(`${missing[0].shortName} could not be loaded for this shared challenge.`);
  const categories = exactDefinitions.map((category) => canonicalizeDataset(datasets.get(category.id)!));
  ...
  if (errors.length) {
    throw new Error(`This exact challenge could not be reconstructed: ${errors[0]}`);
  }
  return { categories, bank: exactBank };
}
...
export default function GeoSecondComingGame() {
  ...
  const exactSpecRef = useRef<ExactRoundSpec | null>(null);
  ...
  function challengeUrl(nextSeed = seed, nextMode = mode, nextRound: Round | null = round) {
    ...
    if (nextRound) {
      url.searchParams.set("roundCats", nextRound.categories.map((dataset) => dataset.category.id).join(","));
      url.searchParams.set("bank", nextRound.bank.map((country) => country.id).join(","));
    }
    return url.toString();
  }
  ...
  async function loadExactRound(nextSeed: string, nextMode: GameMode, spec: ExactRoundSpec, existingCountries = countries) {
    exactSpecRef.current = spec;
    resetRoundState(nextSeed, nextMode);
    setStatus("Loading the exact shared challenge…");
    try {
      const generated = await buildExactRound(existingCountries, spec);
      setRound(generated);
      syncUrl(nextSeed, nextMode, generated);
      setStatus("");
    } catch (e) {
      setError(e instanceof Error ? e.message : "The shared challenge could not be reconstructed.");
      setStatus("");
    }
  }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const exactSpec = parseExactRoundSpec(params);
        if (exactSpec) await loadExactRound(nextSeed, nextMode, exactSpec, list);
        else await loadRound(nextSeed, nextMode, list);
      } catch (e) {
  ...
  function retryCurrentRound() {
    const exactSpec = exactSpecRef.current;
    if (exactSpec) return loadExactRound(seed, mode, exactSpec);
    return mode === "daily" ? playDaily() : loadRound(seed || randomSeed(), "random");
  }
...
${challengeUrl()}`;
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: "Geo: Second Coming", text: shareText });
      } else {
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""}`}>
    ...
    <section className="challengeBar">
      ...
      <div className="challengeActions"><span className="mobileProgress">{Object.keys(assignments).length}/8 assigned</span><button onClick={copyChallenge} disabled={!round}>{copied ? "Link copied ✓" : "Copy link"}</button></div>
    </section>

    <section className="hero">
      <div><span className="kicker">A strategy atlas</span><h2>Ten countries. Eight measures. One perfect allocation.</h2><p>Place eight countries, leave two behind, and make every specialist count. Each challenge link preserves the same board for everyone.</p></div>
