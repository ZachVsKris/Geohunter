...
import { fetchCountries, fetchWorldBankCategory, type CategoryDataset, type CountryInfo } from "../lib/worldBank";
...
async function withTimeout<T>(task: (signal: AbortSignal) => Promise<T>, timeoutMs = FETCH_TIMEOUT_MS): Promise<T> {
...
async function fetchCountryList(): Promise<CountryInfo[]> {
  ...
  try {
    countries = await withTimeout((signal) => fetchCountries(signal), 7_000);
  } catch {
    countries = await withTimeout(async (signal) => {
  ...
  writeClientCache("countries", countries);
  return countries;
}

async function fetchDatasetDirect(category: Category): Promise<CategoryDataset> {
  const cached = readClientCache<CategoryDataset>(`dataset:${category.id}`);
  if (cached) return cached;

  let dataset: CategoryDataset;
  try {
    dataset = await withTimeout((signal) => fetchWorldBankCategory(category, signal), 7_000);
  } catch {
    dataset = await withTimeout(async (signal) => {
      const response = await fetch(`/api/datasets?id=${encodeURIComponent(category.id)}`, { signal });
      const payload = await response.json() as { dataset?: CategoryDataset; error?: string };
      if (!response.ok || !payload.dataset) throw new Error(payload.error ?? `${category.shortName} data could not be loaded.`);
      return payload.dataset;
    }, 9_000);
  }
  writeClientCache(`dataset:${category.id}`, dataset);
  return dataset;
}
...
function findDistinctWinners(
  ...
  if (completeCountries.length < 10) return null;

  const completeIds = new Set(completeCountries.map((country) => country.id));
  const decoyOrder = shuffle(completeCountries, rng);
  ...
  const chosen = new Array<string>(categories.length).fill("");
  const used = new Set<string>();
  let steps = 0;
  const maxSteps = 12_000;
  let result: { winners: string[]; decoys: string[] } | null = null;

  function compatible(categoryA: number, countryA: string, categoryB: number, countryB: string) {
    const aOnA = observationValue(categories[categoryA], countryA);
    const bOnA = observationValue(categories[categoryA], countryB);
    const bOnB = observationValue(categories[categoryB], countryB);
    const aOnB = observationValue(categories[categoryB], countryA);
    return (
      aOnA !== undefined &&
      bOnA !== undefined &&
      bOnB !== undefined &&
      aOnB !== undefined &&
      isBetter(categories[categoryA], aOnA, bOnA) &&
      isBetter(categories[categoryB], bOnB, aOnB)
    );
  }
    ...
    return decoyOrder
      .filter((country) => {
        ...
        return categories.every((category, index) => {
          const winnerValue = observationValue(category, chosen[index]);
  ...
  function search(remaining: number[], options: Map<number, string[]>): boolean {
    if (++steps > maxSteps) return false;
    if (remaining.length === 0) {
      const decoys = findDecoys();
      if (decoys.length < 2) return false;
      result = { winners: [...chosen], decoys };
      return true;
    }

    let categoryIndex = remaining[0];
    for (const index of remaining) {
      if ((options.get(index)?.length ?? 0) < (options.get(categoryIndex)?.length ?? 0)) categoryIndex = index;
    }

    const nextRemaining = remaining.filter((index) => index !== categoryIndex);
    for (const candidateId of options.get(categoryIndex) ?? []) {
      if (used.has(candidateId)) continue;

      let valid = true;
      for (let index = 0; index < chosen.length; index++) {
        if (chosen[index] && !compatible(index, chosen[index], categoryIndex, candidateId)) {
          valid = false;
          break;
        }
      }
      ...
      const nextOptions = new Map<number, string[]>();
      for (const otherIndex of nextRemaining) {
        const filtered = (options.get(otherIndex) ?? []).filter(
          (otherId) => otherId !== candidateId && !used.has(otherId) && compatible(categoryIndex, candidateId, otherIndex, otherId),
        );
        if (filtered.length === 0) {
          valid = false;
          break;
        }
        nextOptions.set(otherIndex, filtered);
      }
      if (!valid) continue;

      chosen[categoryIndex] = candidateId;
      used.add(candidateId);
      if (search(nextRemaining, nextOptions)) return true;
      used.delete(candidateId);
      chosen[categoryIndex] = "";
    }
    return false;
  }

  const initialOptions = new Map<number, string[]>();
  candidates.forEach((list, index) => initialOptions.set(index, list));
  search(categories.map((_, index) => index), initialOptions);
  return result;
}

type LoadProgress = (completed: number, total: number) => void;

async function loadDatasetsByIds(categoryIds: string[], onProgress?: LoadProgress): Promise<RoundCategory[]> {
  ...
  let completed = 0;
  const raw = await mapWithConcurrency(selected, FETCH_CONCURRENCY, async (category) => {
    try {
      return await fetchDatasetDirect(category);
    } catch {
      return null;
    } finally {
      completed += 1;
  ...
  for (const dataset of raw) {
    ...
    try {
      ...
      // Skip a temporarily unavailable or malformed indicator and continue with the rest.
    }
  }
  return loaded;
}

function orderedCandidateCategories(seed: string) {
  const rng = seededRandom(`${seed}:datasets`);
  const familyCounts = new Map<string, number>();
  return shuffle(CATEGORIES.filter((category) => category.enabled !== false), rng).filter((category) => {
    const count = familyCounts.get(category.family) ?? 0;
    if (count >= 4) return false;
    familyCounts.set(category.family, count + 1);
    return true;
  });
}

function intersectCountryIds(current: Set<string> | null, dataset: RoundCategory, validCountryIds: Set<string>) {
  const datasetIds = new Set(dataset.ranked.map((row) => row.countryId).filter((id) => validCountryIds.has(id)));
  if (!current) return datasetIds;
  return new Set([...current].filter((id) => datasetIds.has(id)));
}

function tryBuildRound(countryList: CountryInfo[], seed: string, available: RoundCategory[]): Round | null {
  if (available.length < 8) return null;
  const validCountryIds = new Set(countryList.map((country) => country.id));

  for (let attempt = 0; attempt < 16; attempt++) {
    const rng = seededRandom(`${seed}:attempt:${attempt}`);
    const minimumOverlap = attempt < 6 ? 24 : attempt < 12 ? 16 : 10;
    const categories: RoundCategory[] = [];
    const familyCounts = new Map<string, number>();
    let commonCountryIds: Set<string> | null = null;

    for (const dataset of shuffle(available, rng)) {
      if ((familyCounts.get(dataset.category.family) ?? 0) >= 2) continue;
      const nextCommon = intersectCountryIds(commonCountryIds, dataset, validCountryIds);
      if (nextCommon.size < minimumOverlap) continue;
      categories.push(dataset);
      commonCountryIds = nextCommon;
    ...
    const bank = shuffle(
      [...solution.winners, ...solution.decoys].map((id) => byId.get(id)).filter((country): country is CountryInfo => Boolean(country)),
      rng,
    );
    if (bank.length !== 10) continue;
    if (validateRound(categories, bank).length === 0) return { bank, categories };
  }
  return null;
}

function tryRoundVariants(countryList: CountryInfo[], seed: string, available: RoundCategory[]) {
  for (let variant = 0; variant < 4; variant++) {
    const round = tryBuildRound(countryList, `${seed}:layout:${variant}`, available);
    if (round) return round;
  }
  return null;
}

async function completeGeneratedRound(
  countryList: CountryInfo[],
  seed: string,
  orderedCategories: Category[],
  initialAvailable: RoundCategory[],
  onStatus: (message: string) => void,
): Promise<Round> {
  const availableById = new Map(initialAvailable.map((dataset) => [dataset.category.id, dataset]));
  let cursor = Math.min(10, orderedCategories.length);
  const maximumCandidates = Math.min(40, orderedCategories.length);

  while (true) {
    const available = [...availableById.values()];
    if (available.length >= 8) {
      onStatus("Assembling a balanced board…");
      await new Promise<void>((resolve) => window.setTimeout(resolve, 0));
      const round = tryRoundVariants(countryList, seed, available);
      if (round) return round;
    }
    if (cursor >= maximumCandidates) break;

    const nextCursor = Math.min(maximumCandidates, cursor + (cursor < 18 ? 4 : 6));
    const nextIds = orderedCategories.slice(cursor, nextCursor).map((category) => category.id);
    const additional = await loadDatasetsByIds(nextIds, (completed) => {
      onStatus(`Loading more challenge data… ${cursor + completed}/${maximumCandidates}`);
    });
    for (const dataset of additional) availableById.set(dataset.category.id, dataset);
    cursor = nextCursor;
  }

  throw new Error("The data source did not return enough compatible indicators for this challenge. Try a new round.");
}
...
export default function GeoSecondComingGame() {
  ...
  const [retrySharedBoard, setRetrySharedBoard] = useState<SharedBoard | null>(null);
  ...
  async function loadRound(nextSeed: string, nextMode: GameMode, existingCountries = countries, sharedBoard: SharedBoard | null = null) {
    ...
    setSeed(cleanSeed);
    setMode(nextMode);
    setRetrySharedBoard(sharedBoard);
    ...
    try {
      const countryPromise = existingCountries.length ? Promise.resolve(existingCountries) : fetchCountryList();
      let countryList: CountryInfo[];
      let generated: Round;

      if (sharedBoard) {
        const [loadedCountries, available] = await Promise.all([
          countryPromise,
          loadDatasetsByIds(sharedBoard.categoryIds, (completed, total) => {
            if (loadRequestId.current === requestId) setStatus(`Opening shared challenge… ${completed}/${total}`);
          }),
        ]);
        countryList = loadedCountries;
        generated = buildSharedRound(countryList, sharedBoard, available);
      } else {
        const orderedCategories = orderedCandidateCategories(cleanSeed);
        const initialIds = orderedCategories.slice(0, 10).map((category) => category.id);
        const [loadedCountries, initialAvailable] = await Promise.all([
          countryPromise,
          loadDatasetsByIds(initialIds, (completed, total) => {
            if (loadRequestId.current === requestId) setStatus(`Loading challenge data… ${completed}/${total}`);
          }),
        ]);
        countryList = loadedCountries;
        generated = await completeGeneratedRound(countryList, cleanSeed, orderedCategories, initialAvailable, (message) => {
          if (loadRequestId.current === requestId) setStatus(message);
        });
      }
...
${challengeUrl(seed, mode, round)}`;
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""}`}>
    ...
    {error && <div className="error"><strong>Couldn’t load this round.</strong><span>{error}</span><button onClick={() => retrySharedBoard ? loadRound(seed, mode, countries, retrySharedBoard) : mode === "daily" ? playDaily() : playRandom()}>Try again</button></div>}
