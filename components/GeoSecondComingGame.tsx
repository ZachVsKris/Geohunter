...
import { fetchCategory, fetchCountries, type CountryInfo } from "../lib/worldBank";
...
export default function GeoSecondComingGame() {
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""}`}>
    ...
    {round && scores && <section className="panel results"><div className="score"><span>Final score</span><strong>{total}</strong><p>{total>=650?"Elite allocation.":total>=520?"Strong draft with room to optimize.":"A few specialists were spent in the wrong places."}</p><button className="shareScore" onClick={copyChallenge}>{copied ? "Challenge link copied ✓" : "Challenge a friend"}</button></div>
      ...
      <div className="lock"><span>Maximum score: 800</span><div className="resultActions"><a className="reportLink" href={`mailto:?subject=Geo%20data%20report%20${encodeURIComponent(seed)}&body=${encodeURIComponent(`Seed: ${seed}
Dataset: ${DATASET_VERSION}
Rules: ${RULES_VERSION}
Please describe the possible error:`)}`}>Report possible data error</a><button onClick={()=>playDaily()}>Daily challenge</button><button onClick={()=>playRandom()}>New random round</button></div></div></section>}
