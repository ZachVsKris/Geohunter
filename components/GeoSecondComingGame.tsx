...
${challengeUrl()}`;
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""} ${scores ? "resultsView" : ""}`}>
    ...
    <section className="challengeBar">
      ...
      <div className="challengeActions"><span className="mobileProgress">{Object.keys(assignments).length}/8 assigned</span>{round && <button className="resultsRulesLink" onClick={() => setShowRules(true)}>How it works</button>}<button onClick={copyChallenge} disabled={!round}>{copied ? "Link copied ✓" : "Copy link"}</button></div>
    </section>
