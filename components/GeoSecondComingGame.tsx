...
export default function GeoSecondComingGame() {
  ...
  async function copyChallenge() {
    ...
    const challengeText = `${mode === "daily" ? "Daily challenge" : "Seeded challenge"}: ${mode === "daily" ? seed.replace("DAILY-", "") : seed}`;
    ...
    try {
      ...
      setError("The challenge could not be shared automatically. Copy the URL from your browser instead.");
    }
  }
...
${url}`);
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""} ${scores ? "resultsView" : ""}`}>
    ...
    <section className="challengeBar">
      ...
      <div className="challengeActions"><span className="mobileProgress">{Object.keys(assignments).length}/8 assigned</span>{scores && <button className="resultsRulesLink" onClick={() => setShowRules(true)}>Rules</button>}<button onClick={copyChallenge} disabled={!round}>{copied ? "Shared ✓" : "Share challenge"}</button></div>
    </section>
