...
async function buildRound(countryList: CountryInfo[], seed: string): Promise<Round> {
  ...
  for (const category of shuffle(CATEGORIES, rng)) {
    ...
    if (category.source === "faostat" && (sourceCounts.get(category.source) ?? 0) >= 4) continue;
    try {
      const dataset = canonicalizeDataset(await fetchCategory(category));
      if (dataset.ranked.length < category.coverageFloor) continue;
