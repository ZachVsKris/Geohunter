...
async function fetchExactDataset(category: Category): Promise<RoundCategory> {
  let lastError: unknown;
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      return canonicalizeDataset(await fetchCategory(category));
    } catch (error) {
      lastError = error;
      if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 180 * (attempt + 1)));
    }
  }
  throw lastError instanceof Error ? lastError : new Error(`${category.shortName} could not be loaded.`);
}
...
async function buildExactRound(countryList: CountryInfo[], spec: ExactRoundSpec): Promise<Round> {
  ...
  const categories = await Promise.all(
    definitions.map((category) => fetchExactDataset(category!)),
  );
