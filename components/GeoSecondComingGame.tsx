...
async function buildRound(countryList: CountryInfo[], seed: string): Promise<Round> {
  ...
  // Category requests used to run one at a time. With a large multi-source
  // library that made startup time roughly equal to the sum of every API call.
  // Load small deterministic batches in parallel instead. Promise.allSettled
  // preserves input order, so the same seed still produces the same candidate
  // order regardless of which network response finishes first.
  const BATCH_SIZE = 12;
  const TARGET_AVAILABLE = 16;
  const MAX_CANDIDATES = 48;

  for (let offset = 0; offset < Math.min(shuffledCategories.length, MAX_CANDIDATES); offset += BATCH_SIZE) {
    const batch = shuffledCategories.slice(offset, offset + BATCH_SIZE);
...
export default function GeoSecondComingGame() {
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""}`}>
    ...
    {status && <div className="loading"><div className="spinner"/><strong>{status}</strong><span>Official datasets load in parallel; the first challenge may still take a few seconds, while later rounds reuse cached data.</span></div>}
