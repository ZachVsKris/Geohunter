...
import { CATEGORY_COUNT, COUNTRY_COUNT, DECOY_COUNT, TOP_FINISH_RANK, canAddCategory, measureKind, roundHasRequiredDiversity, roundType } from "../lib/gameRules";
...
function findDistinctWinners(
  ...
  if (decoyCandidates.length < DECOY_COUNT) return null;
  return { winners: winnerByCategory, decoys: decoyCandidates.slice(0, DECOY_COUNT).map((country) => country.id) };
}
...
async function loadCandidateDatasets(seed: string): Promise<RoundCategory[]> {
  ...
  // Load a small, deterministic, broadly mixed candidate pool. The round
  // composer below applies the stricter six-category diversity rules.
  for (let offset = 0; offset < shuffled.length && loaded.length < targetCount; offset += batchSize) {
    ...
    for (const category of shuffled.slice(offset, offset + batchSize * 3)) {
      const type = roundType(category);
      if (loadedIds.has(category.id) || batch.some((item) => item.id === category.id)) continue;
      if ((pendingTypeCounts.get(type) ?? 0) >= 3) continue;
      batch.push(category);
...
function chooseDiverseCategories(available: RoundCategory[], rng: Rng) {
  ...
  while (selected.length < CATEGORY_COUNT) {
    ...
    const scored = options.map((dataset) => ({
      dataset,
      score:
        (selectedTypes.has(roundType(dataset.category)) ? 0 : 12) +
  ...
  return roundHasRequiredDiversity(selected.map((dataset) => dataset.category)) ? selected : null;
}
...
async function buildRound(countryList: CountryInfo[], seed: string): Promise<Round> {
  ...
  if (available.length < CATEGORY_COUNT) {
    throw new Error("Not enough official datasets were available to generate a round. Please try again.");
  }

  const attempted = new Set<string>();
  for (let attempt = 0; attempt < 260; attempt++) {
    const categories = chooseDiverseCategories(available, rng);
    ...
    if (bank.length === COUNTRY_COUNT) {
      const errors = validateRound(categories, bank);
      if (errors.length === 0) return { bank, categories };
    }
  }

  throw new Error("This seed could not produce a balanced six-winner round. Generate another random challenge.");
}
...
export default function GeoSecondComingGame() {
  ...
  const categoryTarget = round?.categories.length ?? CATEGORY_COUNT;
  const poolSize = round?.bank.length ?? COUNTRY_COUNT;
  const roundMaxScore = categoryTarget * 100;
      ...
      if (nextMode === "daily") {
        try {
          const response = await fetch(`/api/daily/${dailyDateFromSeed(nextSeed)}`, {
            ...
            body: JSON.stringify({ seed: nextSeed, encodedBoard: encodeRound(generated) }),
          });
          if (response.ok) {
            const saved = await response.json();
            if (saved.encoded_board) finalRound = decodeRound(saved.encoded_board, existingCountries);
          }
        } catch {
          // The local Daily still works when Supabase has not been configured yet.
        }
      }
      setRound(finalRound);
  ...
  async function shareScore() {
    ...
    const topThree = scores.filter((row) => row.rank <= TOP_FINISH_RANK).length;
    const text = `🌍 GeoStats
${total} / ${roundMaxScore}

🥇 ${firsts}   🥈 ${seconds}   🥉 ${thirds}
⭐ Top 3: ${topThree}/${categoryTarget}
...
${url}`);
  ...
  const topThreeCount = scores?.filter((row) => row.rank <= TOP_FINISH_RANK).length ?? 0;
  ...
  return <div className={`shell ${round && !scores ? "activePlay" : ""} ${scores ? "resultsView" : ""}`}>
    {!scores && <header>
      ...
      <div className="headerButtons"><a href="/audit" className="headerLink">Data audit</a><button onClick={() => setShowRules(true)}>How it works</button><button onClick={() => playDaily()} disabled={!countries.length}>Daily</button><button onClick={() => playRandom()} disabled={!countries.length}>Random</button><AccountControls /></div>
    </header>}
    ...
    {round && scores && <section className="panel results"><div className="score"><span>Final score</span><div className="scoreValue"><strong>{total}</strong><b>/ {roundMaxScore}</b></div><div className="scoreInsights"><div><strong>{averagePlacement}</strong><span>Average placement</span></div><div><strong>{bestPossibleCount}</strong><span>Best possible</span></div><div><strong>{topThreeCount}/{categoryTarget}</strong><span>Top three</span></div></div><div className="scoreBreakdown">{[1,2,3].map((rank)=><span key={rank}>{rank===1?"🥇":rank===2?"🥈":"🥉"} {scores.filter((row)=>row.rank===rank).length}</span>)}</div><p>{total>=roundMaxScore*.8125?"Elite allocation.":total>=roundMaxScore*.65?"Strong draft with room to optimize.":"A few specialists were spent in the wrong places."}</p><div className="scoreActions"><button className="shareScore" onClick={shareScore}>{copied ? "Score copied ✓" : "Share score"}</button><AccountControls results pendingScore={mode === "daily" ? { challengeDate: dailyDateFromSeed(seed), assignments } : undefined} /></div></div>
      ...
      <div className="lock"><span>Maximum score: {roundMaxScore}</span><div className="resultActions"><button onClick={()=>playDaily()}>Daily challenge</button><button onClick={()=>playRandom()}>New random round</button></div></div></section>}
    ...
    {showRules&&<div className="modal" onClick={(e)=>e.currentTarget===e.target&&setShowRules(false)}><div><h2>How GeoStats works</h2><ol><li><strong>Assign six countries.</strong> Each round has 8 countries and 6 categories. Place one country in each category. Two countries will be left unused.</li><li><strong>Read the category direction.</strong> For “Highest,” the largest value ranks first. For “Lowest,” the smallest value ranks first.</li><li><strong>Use each country once.</strong> A country can only be assigned to one category.</li><li><strong>Earn points based on rank.</strong> Your chosen country is ranked against the other seven countries in the round: 1st earns 100 points, 2nd earns 90, 3rd earns 80, and so on down to 30 points for 8th. The highest possible score is <strong>600 points</strong>.</li><li><strong>Every round can be solved perfectly.</strong> Each category has a different country that ranks first among the 8 countries in that round. So all six categories can be matched with a first-place country, and a score of <strong>600 is always possible</strong>.</li></ol><button onClick={()=>setShowRules(false)}>Start drafting</button></div></div>}
  </div>;
}
