...
type TodayLeader = { username: string; displayName: string | null; score: number; averagePlacement: number; firsts: number; topFives: number };
...
export default function LeaderboardView() {
  ...
  return <section className="leaderboardPage panel">
    <div className="leaderboardPageIntro">
      ...
      <h1>Leaderboard</h1>
      <p>Daily games only. Scores are verified against the saved challenge board.</p>
    </div>
    ...
    {loading ? <div className="leaderboardEmpty">Loading leaderboard…</div> : rows.length ? <div className="publicLeaderboard">
      <div className="publicLeaderboardHeader"><span>#</span><span>Player</span>{tab === "today" ? <><span>Score</span><span>Avg. place</span><span>Top 5</span></> : <><span>Average</span><span>Dailies</span><span>Rating</span></>}</div>
      {tab === "today" ? today.map((leader, index) => <div key={`${leader.username}-${index}`}><b>{index + 1}</b><span>{leader.displayName || leader.username}</span><strong>{leader.score}</strong><span>{leader.averagePlacement.toFixed(1)}</span><span>{leader.topFives}/8</span></div>) : alltime.map((leader, index) => <div key={leader.username}><b>{index + 1}</b><span>{leader.displayName || leader.username}</span><strong>{leader.average}</strong><span>{leader.games}</span><span>{leader.rating}</span></div>)}
