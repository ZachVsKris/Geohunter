...
const CATEGORIES = [
  ...
  ["waterStress","Highest agricultural share of freshwater withdrawals","ER.H2O.FWAG.ZS","Environment","high"],
...
async function fetchCategory(category) {
  ...
  for (const row of json?.[1] ?? []) {
    ...
    if (!id || id.length !== 3 || !Number.isFinite(value) || latest.has(id)) continue;
    latest.set(id, { countryId: id, countryName: row.country?.value ?? id, value, year: String(row.date) });
  }
...
async function buildRound(countryList, inputSeed) {
  ...
  if (available.length < 8) throw new Error(`Only ${available.length} usable datasets loaded; eight are required.`);
...
function auditRound(round) {
  ...
  const categories = round.categories.map((dataset) => {
    ...
    if (rows.length !== 10) failures.push(`${dataset.category.name}: expected 10 observations, found ${rows.length}.`);
  ...
  if (winners.size !== 8) failures.push(`Expected eight distinct category winners; found ${winners.size}.`);
  return { passed: failures.length === 0, failures, categories };
}

function printReport(inputSeed, round, report) {
  console.log(`\nGeo: Second Coming audit — ${inputSeed.toUpperCase()}`);
