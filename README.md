# GeoStats v11.2.3 — Active Daily Navigation
...
## Routes and mode navigation

- `geostats.xyz` redirects to the Normal Daily at `/daily`
- Normal Daily: `/daily`
- Expert Daily: `/daily/expert`
- Random challenges: `/random`
- Public leaderboards: `/leaderboard`

Normal and Expert use direct route links so switching modes always loads the requested Daily. The active Daily button stays highlighted. Legacy query-based challenge links at the root remain supported.
...
## v11.2.3 — Daily board dimension repair

- Normal Daily now rejects and repairs legacy 10-country / 8-category rows that were assigned the default `easy` database value during the dual-mode migration.
- Normal is always 8 countries and 6 categories; Expert is always 10 countries and 8 categories.
- Stale or malformed stored Daily boards are ignored client-side and repaired server-side.
- Score verification rejects a board whose dimensions do not match its difficulty.
