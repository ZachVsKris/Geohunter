# GeoStats v11.0.15 — Clear Measures + Atlas Divider

- Audits all 76 current categories so the subtitle explicitly states the comparison basis: total, per person, per 100 people, percentage/share, current US dollars, or physical unit
- Shortens or clarifies several category titles where the previous wording could be misread
- Adds a restrained engraved divider between the Country Bank and The Atlas on desktop
- Strengthens the bottom divider beneath both section headers
- Preserves the nearly equal panel heights and compact mobile layout
- Preserves the removal of the Daily Challenge / Copy Link decorative brackets
- Does not change data, scoring, Supabase, Daily generation, accounts, or leaderboard logic
