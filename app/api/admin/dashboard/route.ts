...
export async function GET(){
 ...
 const [cats,obsCount,imports,sources,boards]=await Promise.all([
  ...
  admin.from('daily_challenges').select('difficulty').eq('challenge_date',new Date().toISOString().slice(0,10))
 ]);
 const errors=[cats.error,obsCount.error,imports.error,sources.error,boards.error].filter(Boolean); if(errors.length)return NextResponse.json({error:errors[0]?.message||'Warehouse query failed.'},{status:500});
 ...
 return NextResponse.json({stats:{categories:cats.count??0,observations:obsCount.count??0,countries:countrySet.size},sources:sources.data??[],imports:imports.data??[],categories:cats.data??[],boards:boardMap});
}
