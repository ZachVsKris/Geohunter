...
export async function POST(request: Request) {
  ...
  const difficulty: DailyDifficulty = body?.difficulty === "expert" ? "expert" : "easy";
  ...
  try {
    ...
    const topFinishRank = difficulty === "expert" ? 5 : 3;
    const topFinishes = rows.filter((row) => row.selected.poolRank <= topFinishRank).length;
