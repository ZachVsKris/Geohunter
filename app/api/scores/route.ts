...
export async function POST(request: Request) {
  ...
  if (!body?.challengeDate || !body.assignments || Object.keys(body.assignments).length !== 8) return NextResponse.json({ error: "Invalid score submission." }, { status: 400 });
  ...
  try {
    ...
    const topFives = rows.filter((row) => row.selected.poolRank <= 5).length;
    ...
    return NextResponse.json({ saved: true, result: data ?? { score, average_placement: averagePlacement, firsts, top_fives: topFives } });
  } catch (error) {
