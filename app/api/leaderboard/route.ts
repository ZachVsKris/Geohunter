...
export async function GET(request: Request) {
  ...
  if (view === "today") query = query.eq("challenge_date", url.searchParams.get("date") || utcDate());
  ...
  if (view === "today") {
    const leaders = rows.map((row) => {
      ...
      return {
        ...
        firsts: row.firsts,
        topFives: row.top_fives,
      };
    }).sort((a, b) => b.score - a.score || a.averagePlacement - b.averagePlacement || b.firsts - a.firsts || b.topFives - a.topFives).slice(0, 100);
    return NextResponse.json({ view, date: url.searchParams.get("date") || utcDate(), leaders }, { headers: { "Cache-Control": "public, s-maxage=30, stale-while-revalidate=120" } });
  }
