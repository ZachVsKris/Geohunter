...
export async function fetchCategory(category: Category): Promise<CategoryDataset> {
  ...
  for (const row of rows) {
    ...
    if (!id || id.length !== 3 || !Number.isFinite(value)) continue;
    if (!latest.has(id)) latest.set(id, { countryId: id, countryName: row.country?.value ?? id, value, year: String(row.date) });
  }
  const observations = [...latest.values()];
  const year = observations.map((o) => o.year).sort().reverse()[0] ?? "Latest available";
