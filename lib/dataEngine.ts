...
import { CATEGORY_COUNT, COUNTRY_COUNT } from "./gameRules";
...
export const POINTS_BY_RANK = [100, 85, 70, 55, 40, 25, 10, 0] as const;
export const LEGACY_POINTS_BY_RANK = [100, 90, 80, 70, 60, 50, 40, 30, 20, 10] as const;
...
export function poolLeaderboard(dataset: CanonicalDataset, bank: CountryInfo[]): PoolRow[] {
  ...
  return sorted.map((row, index) => {
    ...
    lastRank = poolRank;
    const pointsTable = bank.length === 10 ? LEGACY_POINTS_BY_RANK : POINTS_BY_RANK;
...
export function validateRound(categories: CanonicalDataset[], bank: CountryInfo[]) {
  const errors: string[] = [];
  const isCurrentFormat = categories.length === CATEGORY_COUNT && bank.length === COUNTRY_COUNT;
  const isLegacyFormat = categories.length === 8 && bank.length === 10;
  if (!isCurrentFormat && !isLegacyFormat) {
    errors.push(`Expected ${CATEGORY_COUNT} categories with ${COUNTRY_COUNT} countries; found ${categories.length} categories with ${bank.length} countries.`);
  }
