...
export function validateRound(categories: CanonicalDataset[], bank: CountryInfo[]) {
  ...
  for (const dataset of categories) {
    const leaderboard = poolLeaderboard(dataset, bank);
    if (leaderboard.length > 1 && leaderboard[0].poolRank === leaderboard[1].poolRank) {
      errors.push(`${dataset.category.name} does not have a unique first-place country in this pool.`);
    }
